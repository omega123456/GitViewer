...
const DEFAULT_RELEASE_BODY =
  'See release assets for the platform installers and updater packages.';
...
async function main() {
  ...
  const snapshot = {
    ...
    cargoLockRaw: existsSync(cargoLockPath)
      ? readFileSync(cargoLockPath, 'utf8')
      : null,
    releaseBodyRaw: existsSync(releaseBodyPath)
  ...
  if (!parseSemver(currentVersion)) {
    throw new Error(
      `Current Tauri version "${currentVersion}" is not MAJOR.MINOR.PATCH.`,
    );
  }
  ...
  try {
    ...
    if (choice === '1') {
      ...
      const entered = (
        await rl.question('New version (MAJOR.MINOR.PATCH): ')
      ).trim();
    ...
    if (!nextVersion || nextVersion === currentVersion) {
      throw new Error(
        'Could not compute a distinct MAJOR.MINOR.PATCH version.',
      );
    }
    ...
    const cargoTomlUpdated = setCargoPackageVersion(
      snapshot.cargoTomlRaw,
      nextVersion,
    );
