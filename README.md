...
5. Run `pnpm release:version patch` (or `minor`, `major`, or an explicit `X.Y.Z`). It sets the version in `package.json`, `src-tauri/Cargo.toml`, and `src-tauri/tauri.conf.json`, refreshes `src-tauri/Cargo.lock`, commits, tags `vX.Y.Z`, and pushes both.
